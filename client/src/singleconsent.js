...
_GovConsent.prototype.init = function () {
 ...
 var consentConfig = _GovConsentConfig()
  ...
  history.replaceState(null, null, removeUrlParameter(location.href, consentConfig.uidKey))
...
function setUid(consent, uid) {
  ...
  */
  if (uid && uid !== consent.uid) {
    ...
    // fetch known origins
    request(_GovConsentConfig().getApiUrl().replace('/consent/', '/origins/'), {}, function (origins) {
    ...
    // set uid cookie
    document.cookie = _GovConsentConfig().uidKey
      .concat('=', uid)
      .concat(
        '; path=/',
